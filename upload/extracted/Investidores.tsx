...
import { UserPlus, Crown, Shield, Trash2, ArrowUp, ArrowDown, Save } from "lucide-react";
...
export default function Investidores() {
  ...
  const handleAdd = () => {
    if (!nome.trim()) {
      toast.error("Digite o nome do investidor.");
      return;
    }
  ...
  const startReorder = () => {
    const ativos = investidores.filter((i: any) => i.status === "ativo");
    // Ordenar por ordem desc (maior primeiro)
    const sorted = [...ativos].sort((a: any, b: any) => (b.ordem || 0) - (a.ordem || 0));
  ...
  const saveOrder = () => {
    const updates = orderedList.map((item, index) => ({
      id: item.id,
      ordem: orderedList.length - index, // Maior ordem = aparece primeiro
    }));
    reorderInvestidores(updates);
    toast.success("Ordem dos investidores atualizada!");
    setIsReorderMode(false);
  };

  if (isLoading) {
    return <div className="flex items-center justify-center h-64 text-muted-foreground">Carregando investidores...</div>;
  }
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-md border border-primary/30 bg-primary/5 p-4">
        <p className="text-sm text-foreground">
          <strong className="text-primary">Benefício dos Investidores:</strong> Taxa de acréscimo de{" "}
          <strong className="text-green-400">10%</strong> em empréstimos e trocas (vs{" "}
          <strong className="text-yellow-400">15%</strong> para membros comuns).
        </p>
      </div>

      {/* Aviso sobre como obter a tag */}
      <div className="rounded-md border border-yellow-500/30 bg-yellow-500/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-bold text-yellow-400 mb-1">Como obter a tag de Investidor?</p>
            <p className="text-sm text-foreground">
              Para receber a tag de <strong className="text-yellow-400">Investidor</strong>, o player precisa ajudar o banco de alguma forma — seja <strong className="text-foreground">doando itens</strong>, <strong className="text-foreground">fazendo trocas</strong> ou <strong className="text-foreground">ajudando de qualquer outra maneira</strong> que fortaleça o banco do clã.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-md border border-border bg-card p-4">
        <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
          <UserPlus className="w-4 h-4 text-primary" /> Adicionar Investidor
        </h3>
        <div className="flex gap-3">
          <div className="flex-1">
            <Label className="text-xs text-muted-foreground">Nome do Player</Label>
            <Input
          ...
          <div className="flex items-end">
            <Button onClick={handleAdd} className="bg-primary hover:bg-primary/90 text-primary-foreground">
              <Crown className="w-4 h-4 mr-1" /> Adicionar
            </Button>
          </div>
        </div>
      </div>
      ...
      {canEdit && !isReorderMode && (
        ...
        >
          <ArrowUp className="w-4 h-4 mr-1" /> Reordenar Ranking
        </Button>
      )}
      ...
      {canEdit && isReorderMode && (
        <div className="rounded-md border border-primary/30 bg-card p-4">
          <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">
            <ArrowUp className="w-4 h-4 text-primary" /> Reordenar Investidores (arraste para cima/baixo)
          </h3>
          <div className="space-y-2">
            {orderedList.map((inv, index) => (
              <div key={inv.id} className="flex items-center gap-2 p-2 rounded-md border border-border bg-muted/30">
                ...
                <div className="flex items-center gap-2 flex-1">
                  ...
                  <span className="text-sm font-semibold text-foreground">{inv.nome}</span>
                  <span className="text-xs text-muted-foreground ml-auto">
                    Desde {new Date(inv.dataEntrada).toLocaleDateString("pt-BR")}
                  </span>
                </div>
              </div>
            ))}
      ...
      <div>
        <h3 className="text-sm font-bold text-foreground mb-2 flex items-center gap-2">
          <Crown className="w-4 h-4 text-yellow-400" /> Investidores Ativos ({ativos.length})
        </h3>
        {ativos.length === 0 ? (
            ...
            {ativos.map((inv: any, idx: number) => (
              ...
              >
                <div className="flex items-center gap-2">
                  <div className="w-6 h-6 flex items-center justify-center">
                    {idx === 0 ? <Crown className="w-5 h-5 text-yellow-400" /> :
                     idx === 1 ? <span className="text-sm font-bold text-gray-300">2</span> :
                     idx === 2 ? <span className="text-sm font-bold text-orange-600">3</span> :
                     <span className="text-xs font-bold text-muted-foreground">#{idx + 1}</span>}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-foreground">{inv.nome}</p>
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => removeInvestidor(inv.id)}
