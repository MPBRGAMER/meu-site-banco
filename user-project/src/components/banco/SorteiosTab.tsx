  ...
  return (
    <div className="rounded-lg border border-primary/20 bg-card overflow-hidden">
      ...
      <div className="p-4">
        ...
        {participantes.length > 0 && <div className="mb-3 max-h-20 overflow-y-auto space-y-1">{participantes.map((p) => (<div key={p.id} className="flex items-center gap-2 text-xs"><Users className="w-3 h-3 text-primary" /><span className="text-foreground" data-no-translate>{p.jogador}</span></div>))}</div>}
        <div className="mt-3">
          {showPart ? (
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="text-right">
            ...
            <p className="text-sm font-bold text-yellow-400" data-no-translate>{entry.ganhador || "-"}</p>
          </div>
      ...
      {expanded && (
        <div className="border-t border-border bg-muted/20 p-3">
          ...
          <div className="rounded-md border border-yellow-500/30 bg-yellow-500/5 p-3 mb-3">
            ...
            <p className="text-base font-bold text-yellow-400 flex items-center gap-2" data-no-translate><Trophy className="w-4 h-4" /> {entry.ganhador || "Nenhum"}</p>
          </div>
              ...
              <div className="max-h-32 overflow-y-auto space-y-1">
                {participantes.map((p) => (
                  <div key={p.id} className={`flex items-center gap-2 text-xs py-1 px-2 rounded ${p.jogador === entry.ganhador ? "bg-yellow-500/10 border border-yellow-500/30" : ""}`}>
                    ...
                    <span className={p.jogador === entry.ganhador ? "text-yellow-400 font-bold" : "text-foreground"} data-no-translate>{p.jogador}</span>
                  </div>
                ))}
              </div>
            </div>
