  ...
  try {
    ...
    switch (action) {
      ...
      case "removeDoador": {
        const { id } = data;
        if (!id) return err("id obrigatório");
        // Buscar doador para estornar estoque
      ...
      case "backup": {
        ...
        const [emprestimos, investidores, tabelasTroca, trocas, comprasVendas, caixa, doadores, leiloes, lances, sorteios, participantes, lotericas, numeros, priceReports, itemOverrides, propagandas, chatSalas, chatMensagens] = await Promise.all([
          ...
          db.lance.findMany(),
          db.sorteio.findMany(),
          db.participanteSorteio.findMany(),
          db.loterica.findMany(),
          db.numeroLoterica.findMany(),
          db.priceReport.findMany(),
          db.itemOverride.findMany(),
          db.propaganda.findMany(),
          db.chatSala.findMany(),
          db.chatMensagem.findMany(),
        ]);
