...
const SUPER_ADMIN_ONLY = new Set(["resetAll","backupDB","restoreBackup","bulkInsertDoadores","createModerador","updateModerador","removeModerador","setupModeradores"]);
      ...
      case "backupDB": {
        ...
        try {
          const results = await Promise.all([
            db.emprestimo.findMany().catch(() => []), db.investidor.findMany().catch(() => []),
            db.tabelaTroca.findMany().catch(() => []), db.trocaRegistro.findMany().catch(() => []),
            db.compraVenda.findMany().catch(() => []), db.caixaRegistro.findMany().catch(() => []),
            db.doador.findMany().catch(() => []), db.leilao.findMany().catch(() => []),
            db.lance.findMany().catch(() => []), db.sorteio.findMany().catch(() => []),
            db.participanteSorteio.findMany().catch(() => []), db.loterica.findMany().catch(() => []),
            db.numeroLoterica.findMany().catch(() => []), db.priceReport.findMany().catch(() => []),
            db.itemOverride.findMany().catch(() => []), db.propaganda.findMany().catch(() => []),
            db.chatSala.findMany().catch(() => []), db.chatMensagem.findMany().catch(() => []),
            db.itemCatalogo.findMany().catch(() => []), db.moderador.findMany().catch(() => []),
          ]);
